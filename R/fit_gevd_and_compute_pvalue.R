...
parser$add_argument('-g', '--gps', type = 'numeric', help = 'GPS statistic computed using non-permuted data', required = T)
...
write.table(data.frame(gps = args$gps, n = nrow(perm_dat), loc = loc, loc.sd = loc.sd, scale = scale, scale.sd = scale.sd, shape = shape, shape.sd = shape.sd, pval = pvalue), sep = '\t', file = args$output_file)
